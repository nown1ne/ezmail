...
<div class="flex items-center justify-center gap-5 md:gap-10 p-2">

    
    <div class="">
        <input bind:value={id}  type="text" id="email" class="border text-gray-900 text-sm md:text-lg rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full  px-3 md:px-6 py-4 bg-gray-900 border-primary-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="example@mail.com" required>
    </div> 
    ...
    <div on:click={send} class="relative inline-flex items-center justify-start py-3 pl-4 pr-12 overflow-hidden font-semibold text-primary-400 transition-all duration-150 ease-in-out rounded hover:pl-10 hover:pr-6 bg-gray-900` group">
        ...
        <span class="relative text-sm md:text-lg w-full text-left transition-colors duration-200 ease-in-out group-hover:text-white">Send Mail</span>
    </div>
